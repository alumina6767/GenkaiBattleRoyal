#> battle:item/bits/throw/then/1000
# bitsを投げる
# @internal

#>
# @within battle:item/bits/throw/**
    #declare tag 1000bits

## ベクトル計算用
execute positioned 0.0 0.0 0.0 run summon marker ^ ^ ^0.1 {UUID: [I; 6, 7, 0, 7]}

## 飛ばすファイアーボール
summon fireball ^ ^ ^2.0 {ExplosionPower:0b,Tags:["new","1000bits","bits"],Passengers:[{id:"minecraft:area_effect_cloud",Radius:0f,Duration:10000,Tags:["1000bits","bits","new"]}],Item:{id:"ender_eye",tag:{CustomModelData:670001},Count:1b}}

## 重力時
execute if predicate share:is_sneak as 00000006-0000-0007-0000-000000000007 at @s run tp @s ~ ~0.2 ~
execute if predicate share:is_sneak run scoreboard players set @e[type=fireball,tag=new] tmp 20
