...
execute if entity @s[tag=barrier.blockable] positioned ^ ^ ^2 as @e[tag=!barrier.effect,predicate=!battle:same_team,predicate=battle:can_take_damage,distance=..2] unless entity @s[scores={hummer2.damage_cd=1..}] at @s run function battle:item/hummer2/attack
